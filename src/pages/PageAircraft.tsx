...
} from "../redux/reducers/aircraftReducer";
...
const PageAircraft: React.FC = () => {
  ...
  const handleTableChange = (newPage: number, newPageSize: number) => {
    dispatch(fetchAircraftRequest({ page: newPage, pageSize: newPageSize }));
  };

  const handleSave = (key: any, data: AppAircraft) => {
    if (data.uuid) {
      return dispatch(updateAircraftRequest(data));
    }
    dispatch(createAircraftRequest(data));
  };

  const handleDelete = (uuid: string) => {
    dispatch(deleteAircraftRequest(uuid));
  };
  ...
  const columns: ProColumns<AppAircraft>[] = [
    {
      title: "Modificado",
