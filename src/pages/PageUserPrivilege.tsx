...
} from "../redux/reducers/userPrivilegeReducer";
...
import { fetchModuleRequest } from "../redux/reducers/moduleReducer";

const PageUserPrivilege: React.FC = () => {
  const dispatch = useDispatch();
  const { data, loading, pagination } = useSelector(
    (state: ReduxState) => state.userPrivilege
  );
  const { user } = useSelector((state: ReduxState) => state.login);
  const { data: userRoleData } = useSelector(
    (state: ReduxState) => state.userRole
  );
  const { data: moduleData } = useSelector((state: ReduxState) => state.module);
  const [editableKeys, setEditableRowKeys] = useState<React.Key[]>([]);
  const [position, setPosition] = useState<"top" | "bottom" | "hidden">("top");
  const refreshData = () => {
    dispatch(fetchUserPrivilegeRequest({ page: 1, pageSize: Config.pageSize }));
    dispatch(fetchUserRoleRequest({ page: 1, pageSize: 100 }));
    dispatch(fetchModuleRequest({ page: 1, pageSize: 100 }));
  };
  ...
  const handleTableChange = (newPage: number, newPageSize: number) => {
    dispatch(
      fetchUserPrivilegeRequest({ page: newPage, pageSize: newPageSize })
    );
  };

  const handleSave = (key: any, data: AppUserPrivilege) => {
    if (data.uuid) {
      return dispatch(updateUserPrivilegeRequest(data));
    }
    dispatch(createUserPrivilegeRequest(data));
  };

  const handleDelete = (uuid: string) => {
    dispatch(deleteUserPrivilegeRequest(uuid));
  };
  ...
  const columns: ProColumns<AppUserPrivilege>[] = [
    {
      title: "Rol",
      dataIndex: "roleId",
      valueType: "select",
    ...
    },
    {
      ...
      width: "15%",
      sorter: (a, b) => a.moduleId - b.moduleId,
    },
    {
      title: "Modificado",
  ...
  return (
    <>
      <EditableProTable<AppUserPrivilege>
        rowKey="id"
...
export default PageUserPrivilege;
