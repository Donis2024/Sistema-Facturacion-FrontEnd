...
} from "../redux/reducers/itineraryReducer";
...
const PageItinerary: React.FC = () => {
  ...
  const handleTableChange = (newPage: number, newPageSize: number) => {
    dispatch(fetchItineraryRequest({ page: newPage, pageSize: newPageSize }));
  };

  const handleSave = (key: any, data: AppItinerary) => {
    if (data.uuid) {
      return dispatch(updateItineraryRequest(data));
    }
    dispatch(createItineraryRequest(data));
  };

  const handleDelete = (uuid: string) => {
    dispatch(deleteItineraryRequest(uuid));
  };
  ...
  const columns: ProColumns<AppItinerary>[] = [
    {
      title: "Modificado",
  ...
  return (
    <>
      <EditableProTable<AppItinerary>
        rowKey="id"
        headerTitle="Itinerarios"
        pagination={{
